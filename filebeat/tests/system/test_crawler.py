...
class Test(BaseTest):
    ...
    def test_fetched_lines(self):
        ...
        with open(testfile, 'w') as file:
            ...
            for n in range(iterations):
                file.write(f"hello world{str(n)}")
                file.write("\n")
    ...
    def test_file_renaming(self):
        ...
        testfile1 = f"{self.working_dir}/log/test-old.log"
        with open(testfile1, 'wb', buffering=0) as file:
            iterations1 = 5
        ...
        testfile2 = f"{self.working_dir}/log/test-new.log"
        os.rename(testfile1, testfile2)
        with open(testfile2, 'a') as file:
    ...
    def test_file_disappear_appear(self):
        ...
        self.render_config_template(
            path=f"{os.path.abspath(self.working_dir)}/log/*.log",
            close_removed="true",
            scan_frequency="0.1s",
            clean_removed="false",
        )
        ...
        testfile = f"{self.working_dir}/log/test.log"
        testfilenew = f"{self.working_dir}/log/hiddenfile"
        with open(testfile, 'w') as file:
    ...
    def test_multiple_appends(self):
        ...
        for n in range(3):
            with open(testfile, 'a') as f:
                ...
                for i in range(20 + n):
                    f.write(f"hello world {str(i)} {str(n)}" + "\n")
        ...
        # Check that output file has the same number of lines as the log file
        output = self.read_output()
        assert len(output) == 3 * 20 + sum(range(3)) + 1
    ...
    def test_tail_files(self):
        ...
        self.render_config_template(
            path=f"{os.path.abspath(self.working_dir)}/log/*", tail_files="true"
        )
        os.mkdir(f"{self.working_dir}/log/")
    ...
    def test_utf8(self):
        ...
        self.render_config_template(
            path=f"{os.path.abspath(self.working_dir)}/log/*", encoding="utf-8"
        )
        os.mkdir(f"{self.working_dir}/log/")
    ...
    def test_encodings(self):
        ...
        for _, enc_py, text in encodings:
            with codecs.open(f"{self.working_dir}/log/test-{enc_py}", "w", enc_py) as f:
                f.write(text + "\n")

        inputs = [
            {"path": f"{self.working_dir}/log/test-{enc_py}", "encoding": enc_go}
        ...
        for _, enc_py, text in encodings:
            with codecs.open(f"{self.working_dir}/log/test-{enc_py}", "a", enc_py) as f:
                f.write(f"{text} 2" + "\n")
    ...
    def test_include_lines(self):
        ...
        with open(testfile, 'w') as file:
            iterations = 20
            for n in range(iterations):
                file.write(f"DBG: a simple debug message{str(n)}")
                file.write("\n")
                file.write(f"ERR: a simple error message{str(n)}")
                file.write("\n")
                file.write(f"WARNING: a simple warning message{str(n)}")
                file.write("\n")
    ...
    def test_include_exclude_lines(self):
        ...
        with open(testfile, 'w') as file:
            ...
            for n in range(iterations):
                file.write(f"DBG: a simple debug message{str(n)}")
                file.write("\n")
                file.write(f"ERR: apache simple error message{str(n)}")
                file.write("\n")
                file.write(f"ERR: a simple warning message{str(n)}")
                file.write("\n")
